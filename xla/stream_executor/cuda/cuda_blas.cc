...
bool CUDABlas::DoBlasGemv(Stream *stream, blas::Transpose trans, uint64_t m,
  ...
  return DoBlasInternal(cublasCgemv, stream, true /* = pointer_mode_host */,
                        ...
                        CUDAComplex(GpuMemory(a)), lda, CUDAComplex(GpuMemory(x)),
                        incx, CUDAComplex(&cb_beta),
  ...
  switch (dtype) {
    ...
    case dnn::kComplexFloat: {
      ...
      return DoBlasInternalImpl(
          ...
          static_cast<const cuComplex *>(a.opaque()), lda, stride_a,
          static_cast<const cuComplex *>(b.opaque()), ldb, stride_b,
          CUDAComplex(&cb_beta), static_cast<cuComplex *>(c->opaque()), ldc,
          stride_c, batch_count);
    }
    case dnn::kComplexDouble: {
      cuDoubleComplex cb_alpha =
      ...
      return DoBlasInternalImpl(
          ...
          CUDAComplex(&cb_beta),
          static_cast<cuDoubleComplex *>(c->opaque()), ldc, stride_c,
          batch_count);
    }
    default:
      return absl::InternalError(absl::StrCat("Unsupported datatype for GEMM: ",
